...
describe Hand do
  ...
  let(:straight_flush_2) { Hand.new([spades_2, spades_3, spades_4, spades_5, spades_6]) }
  let(:four_of_a_kind_2) { Hand.new([hearts_8, spades_8, clubs_8, diamonds_8, hearts_9]) }
  let(:full_house_2) { Hand.new([hearts_8, spades_8, clubs_8, diamonds_9, hearts_9]) }
  let(:flush_2) { Hand.new([spades_2, spades_3, spades_4, spades_5, spades_6]) }
  let(:straight_2) { Hand.new([spades_2, spades_3, spades_4, spades_5, spades_6]) }
  let(:three_of_a_kind_2) { Hand.new([hearts_10, spades_10, clubs_9, diamonds_10, hearts_8]) }
  let(:two_pair_2) { Hand.new([clubs_7, diamonds_7, clubs_9, diamonds_10, hearts_9]) }
  let(:one_pair_2) { Hand.new([clubs_7, diamonds_7, clubs_9, diamonds_10, hearts_11]) }
  let(:random_hand_2) { Hand.new([hearts_8, spades_2, clubs_11, diamonds_5, hearts_3]) }
  ...
  describe "#beats_hand?" do
    ...
    it "tests for the better of two straight flushes" do
      expect(straight_flush.beats_hand?(straight_flush_2)).to be false
    end

    it "tests for the better of two two-pairs" do
      expect(two_pair.beats_hand?(two_pair_2)).to be false
    end

    it "tests for the better of two high-card hands" do
      expect(random_hand.beats_hand?(random_hand_2)).to be false
    end

    # could add more tests here but
